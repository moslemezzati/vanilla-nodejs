...
workers.gatherAllChecks = () => {
    _file.list(CHECKS_FOLDER, (err, files) => {
        if (err || !files || files.length < 1) {
            return debug('Could not find any checks to process');
        }
        files.forEach(file => {
            _file.read(CHECKS_FOLDER, file, (err, originalCheckData) => {
                if (err || !originalCheckData) {
                    return debug(`Could not reading this file: ${originalCheckData}`);
                }
...
workers.validateCheckData = (originalCheckData) => {
    ...
    if (!originalCheckData.id ||
        ...
        return debug('Error: One of the checks is not properly formatted!');
    }
...
workers.processCheckOutcome = (originalCheckData, checkOutcome) => {
    ...
    _file.update(CHECKS_FOLDER, originalCheckData.id, originalCheckData, err => {
        if (err) {
            return debug('Error: Tring to update file with id:', originalCheckData.id);
        }
        if (alertWarranted) {
...
workers.alertUserToStatusChange = (checkData) => {
    ...
    helpers.sendTwilioSMS(checkData.userPhone, msg, err => {
        if (err) {
            return debug('Error: Could not send sms alert to user who had a state change in their check.', err);
        }
        debug('Success: User was alerted to a status change in thire check via sms:', msg)
    });
};
    ...
    _log.list(false, (err, logs) => {
        ...
        logs.forEach(log => {
            ...
            _log.compress(logId, newFileId, err => {
                if (err) {
                    return debug('Error compressing ', logId, 'file');
                }
                _log.truncate(logId, err => {
                    if (err) {
                        return debug('Error truncating log file');
                    }
                    debug('Success truncation log file');
                });
            });
        });
    });
